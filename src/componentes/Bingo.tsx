...
const Bingo: React.FC = () => {
    ...
    const { numbers, lastThreeNumbers, generateRandomNumber, resetGame } = useBingoLogic();
    ...
    return (
        <>
            <div className="flex flex-col items-center justify-start min-h-screen bg-gray-100 p-10">
                ...
                <div>
                    <h1 className='text-normal sm:text-1xl md:text-2xl lg:text-3xl xl:text-4xl mt-8'>Total a jugar: {total} soles</h1>
                </div>
                            ...
                            return (
                                <div
                                    ...
                                    className={`w-9 h-9 sm:w-12 sm:h-12 md:w-14 md:h-14 lg:w-16 lg:h-16 xl:w-18 xl:h-18 flex items-center justify-center rounded-full
                                     text-white font-semibold text-2xl ${numbers.includes(num) ? 'bg-yellow-600' : 'bg-gray-400'
                                        }`}
                                >
                                    {num}
                                </div>
